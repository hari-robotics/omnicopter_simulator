...
public:
    ...
    void OnUpdate() {
        for (std::vector<JointValue>::iterator joint = joints_.begin(); joint != joints_.end(); joint++) {
            ...
            auto w_prop = joint->ptr->GetChild()->WorldAngularVel();
            auto pos_prop = joint->ptr->GetChild()->WorldPose().Pos();
            auto C_t = joint->torque_constant;
            auto C_f = joint->force_constant;

            // Force and torque relative to the propeller link
            auto f_world = C_f * w_prop * w_prop.Abs() * joint->dir_sign_;
            auto t_world = -C_t * w_prop * w_prop.Abs();

            // Apply force and torque to the connected parent link
            joint->ptr->GetParent()->AddForceAtWorldPosition(f_world,pos_prop);
            joint->ptr->GetParent()->AddTorque(t_world);
